...
export function runMachineInternal (machine: StateMachine, input: Array<string>, partial: boolean = false) {
	...
	for (let t = 0; t < tokens.length; ++t) {
			...
			if (!partial || t < tokens.length - 1 || hasExactMatch) {
				if (hasExactMatch) {
					...
					for (const { to, reducer } of transitions) {
						nextBranches.push({
							node: to,
							state: typeof reducer !== 'undefined' ? execute(reducers, reducer, state, segment) : state,
						});
				...
				for (const candidate of Object.keys(nodeDef.statics)) {
					...
					if (segment === candidate) {
						...
						for (const { to } of nodeDef.statics[candidate]) {
							nextBranches.push({
								node: to,
								state: { ...state, remainder: candidate.slice(segment.length) },
							});
		...
		if (nextBranches.length === 0) {
			throw new errors.UnknownSyntaxError(
				input,
				branches
				.filter(({ node }) => {
...
function checkIfNodeIsFinished(node: Node, state: RunState) {
	...
	if (Object.prototype.hasOwnProperty.call(node.statics, END_OF_INPUT))
		for (const { to } of node.statics[END_OF_INPUT]) if (to === NODE_SUCCESS) return true;
...
function suggestMachine(machine: StateMachine, input: Array<string>, partial: boolean) {
	...
	const traverseSuggestion = (suggestion: Array<string>, node: number, skipFirst: boolean = true) => {
			...
			for (const node of currentNodes) {
				...
				for (const key of Object.keys(nodeDef.statics)) {
					...
					for (const { to, reducer } of nodeDef.statics[segment]) {
						if (reducer !== 'pushPath') continue;

						if (!skipFirst) suggestion.push(segment);
		...
		if (suggestionsJson.has(json)) return;
	...
	for (const { node, state } of branches) {
		...
		for (const [candidate, transitions] of Object.entries(nodeDef.statics))
			if (
				(isFinished && candidate !== END_OF_INPUT) ||
				(!candidate.startsWith(`-`) && transitions.some(({ reducer }) => reducer === 'pushPath'))
			)
...
export function selectBestState (input: Array<string>, states: Array<RunState>) {
	const terminalStates = states.filter((state) => state.selectedIndex !== null);
	...
	const requiredOptionsSetStates = terminalStates.filter((state) =>
		state.requiredOptions.every((names) => (
			names.some((name) => state.options.find((opt) => opt.name === name))
		))
	);
	...
	const bestPathBranches = requiredOptionsSetStates.filter((state) => state.path.length === maxPathSize);

	const getPositionalCount = (state: RunState) => (
		state.positionals.filter(({ extra }) => !extra).length + state.options.length
	);
	...
	const bestPositionalStates = statesWithPositionalCount
		.filter(({ positionalCount }) => {
	...
	if (fixedStates.length > 1) {
		throw new errors.AmbiguousSyntaxError(
			input,
			fixedStates.map((state) => state.candidateUsage!)
		);
	}
...
function findCommonPrefix (firstPath: Array<string>, secondPath: Array<string> | undefined, ...rest: Array<Array<string>>): Array<string> {
	if (secondPath === undefined) return Array.from(firstPath);
...
export function registerDynamic<T extends keyof typeof tests, R extends keyof typeof reducers>(
	machine: StateMachine,
	from: number,
	test: Callback<T, typeof tests>,
	to: number,
	reducer?: Callback<R, typeof reducers>
) {
...
export function registerShortcut<R extends keyof typeof reducers>(
	machine: StateMachine,
	from: number,
	to: number,
	reducer?: Callback<R, typeof reducers>
	...
	machine.nodes[from].shortcuts.push({
		to,
		reducer: reducer as Callback<keyof typeof reducers, typeof reducers>,
	});
}

export function registerStatic<R extends keyof typeof reducers>(
	machine: StateMachine,
	from: number,
	test: string,
	to: number,
...
export type CallbackFn<P extends Array<any>, R> =
	(state: RunState, segment: string, ...args: P) => R;

export type CallbackFnParameters<T extends CallbackFn<any, any>> =
	T extends (state: RunState, segment: string, ...args: infer P) => any ? P : never;
...
export function execute<T extends string, R, S extends CallbackStore<T, R>>(
store: S,
callback: Callback<T, S>,
state: RunState,
segment: string
) {
...
export function suggest (
	callback: Callback<keyof typeof tests, typeof tests>,
	state: RunState
): Array<string> | null {
...
export const tests = {
	...
	isBatchOption: (state: RunState, segment: string, names: Array<string>) => {
		return (
			!state.ignoreOptions &&
			BATCH_REGEX.test(segment) &&
			[...segment.slice(1)].every((name) => names.includes(`-${name}`))
		);
	},
	isBoundOption: (state: RunState, segment: string, names: Array<string>, options: Array<OptDefinition>) => {
		...
		return (
			!state.ignoreOptions &&
			!!optionParsing &&
			OPTION_REGEX.test(optionParsing[1]) &&
...
export const reducers = {
	...
	pushBatch: (state: RunState, segment: string) => {
		return {
			...state,
			options: state.options.concat([...segment.slice(1)].map((name) => ({ name: `-${name}`, value: true }))),
		};
	},
...
export class CommandBuilder<Context> {
	...
	usage ({ detailed = true, inlineOptions = true }: { detailed?: boolean; inlineOptions?: boolean } = {}) {
		...
		const detailedOptionList: Array<{ definition: string; description: string; required: boolean }> = [];
		...
		for (const option of this.options) {
			...
			if (option.arity === 0) {
				...
				for (let t = 0; t < option.arity; ++t) {
					...
					const action: keyof typeof reducers = option.arity === 1 ? 'setStringValue' : 'pushStringValue';
