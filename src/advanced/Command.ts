...
import { isRecord, isUnknown, cascade } from 'typanion';
...
export type Definition = Usage & {
	 ...
	 */
	options: Array<{ definition: string; description?: string; required: boolean }>;
};
...
export abstract class Command<Context extends BaseContext = BaseContext> {
	 ...
	 *
	 * Expected to return an exit code or nothing (which it will treat as if 0
	 * had been returned).
	 */
	 ...
	 * be used to query Cli and forward commands.
	 */
	cli!: MiniCli<Context>;
	...
	async validateAndExecute (): Promise<number> {
		...
		const commandSchema = commandClass.schema;

		if (Array.isArray(commandSchema)) {
			const schema = cascade(isRecord(isUnknown()), commandSchema);
